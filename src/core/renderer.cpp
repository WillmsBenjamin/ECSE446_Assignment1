...
void Renderer::render() {
    if (realTime) {
		...
		float pixelScaling = tan(deg2rad * scene.config.camera.fov / 2.f) * 2.f / scene.config.height;
		float rightX = (pixelScaling * scene.config.width) / 2.f;
		float bottomY = (pixelScaling * scene.config.height) / 2.f;

		integrator->init();
		integrator->samplers.clear();
		chrono::milliseconds ms = chrono::duration_cast< chrono::milliseconds >(
			chrono::system_clock::now().time_since_epoch()
			);
		Sampler* sampler = new Sampler(ms.count());

		integrator->samplers.push_back(*sampler);
		v4f pixel, dir4;
		v3f tempColor, finalColor, dir3;
		Ray* ray = new Ray(scene.config.camera.o, scene.config.camera.o);
		pixel.z = 1;
		pixel.w = 1;

		finalColor.x = 0.f; finalColor.y = 0.f; finalColor.z = 0.f;
			...
			for (int y = 0; y < scene.config.height; y++) {
				if (scene.config.spp > 1) {
					for (int i = 0; i < scene.config.spp; i++) {
						pixel.x = rightX - ((x * pixelScaling) + (pixelScaling * sampler->next()));
						pixel.y = (y * pixelScaling) + (pixelScaling * sampler->next()) - bottomY;
						dir4 = pixel * inverseView;

						dir3.x = dir4.x * dir4.w;
						dir3.y = dir4.y * dir4.w;
						dir3.z = dir4.z * dir4.w;

						ray->d = dir3;
						tempColor = integrator->render(*ray, *sampler);
						finalColor.x += tempColor.x; finalColor.y += tempColor.y; finalColor.z += tempColor.z;
					}
					finalColor.x /= scene.config.spp;
					finalColor.y /= scene.config.spp;
					finalColor.z /= scene.config.spp;
				}
				else {
					pixel.x = rightX - ((x * pixelScaling) + (pixelScaling / 2.f));
					pixel.y = (y * pixelScaling) + (pixelScaling / 2.f) - bottomY;
					dir4 = pixel * inverseView;
					...
					ray->d = dir3;
					finalColor = integrator->render(*ray, *sampler);
				}

				integrator->rgb->data[x + (y * scene.config.width)] = finalColor;
			}
		}
    }
}
